...
static gboolean webKitMediaSrcDemuxerSinkEventWithParent(GstPad* pad, GstObject* parent, GstEvent* event)
{
    ...
    case GST_EVENT_FLUSH_START:
        // No break, will fallback to default
        printf("### %s: Flush-start at the input of the demuxer %s\n", __PRETTY_FUNCTION__, GST_ELEMENT_NAME(GST_ELEMENT(parent))); fflush(stdout);

    case GST_EVENT_FLUSH_STOP: {
        printf("### %s: Flush-stop at the input of the demuxer %s\n", __PRETTY_FUNCTION__, GST_ELEMENT_NAME(GST_ELEMENT(parent))); fflush(stdout);
        ...
        if (result) {
            ...
            if (nextSamplePts.isInvalid())
                nextSamplePts = MediaTime(double(0));

            // !!!
            printf("### %s [%d]: Pushing artificial [%f, Inf] segment to the %s demuxer\n", __PRETTY_FUNCTION__, WTF::currentThread(), nextSamplePts.toDouble(), GST_ELEMENT_NAME(GST_ELEMENT(parent))); fflush(stdout);
            ...
            gst_element_send_event(GST_ELEMENT(parent), segmentEvent);
...
bool MediaSourceClientGStreamer::append(PassRefPtr<SourceBufferPrivateGStreamer> sourceBufferPrivate, const unsigned char* data, unsigned length)
{
    ...
    printf("### %s: %s %u bytes\n", __PRETTY_FUNCTION__, GST_ELEMENT_NAME(source->src), length); fflush(stdout);
    ...
    if (!ok) {
        GST_OBJECT_LOCK(m_src.get());
        m_src.get()->priv->isAppending = FALSE;
        GST_OBJECT_UNLOCK(m_src.get());
    }
...
//        stream->parser####
    ...
    if (!demuxersrcpad) {
        printf("### %s: demuxersrcpad not found\n", __PRETTY_FUNCTION__); fflush(stdout);
        if (segment)
            gst_segment_free(segment);
        return;
    }
